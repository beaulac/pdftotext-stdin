...
function _runExtraction(pdfStream, pipePath, pdf2TxtArgs) {
    return Promise.using(
        namedPipes.createDisposableFifoReadStream(pipePath),
        pipeRs => {
...
function extractTextFromPdfStream(pdfStream, options = {}) {
    ...
    return Promise.using(
        outputPath,
        (pipePath) => _runExtraction(pdfStream, pipePath, [...pdf2TxtArgs, pipePath])
    );
}
