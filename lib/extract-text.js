...
function execPdfToText(pdfStream, args, pipeRs) {
    return new Promise(
        (resolve, reject) => {
            _debug('Spawning ', pdf2TxtCmd, args.join(' '));
            const pdf2TxtProc = spawn(
                pdf2TxtCmd,
                args,
                { stdio: ['pipe', 'pipe', 'pipe'] }
            );

            // Why doesn't it work to just pass the socket as stdout? Strange.
            pipe(pdf2TxtProc.stdout, pipeRs, err => {
                if (err) {
...
function _promiseRead(rs) {
    let onData, onErr, onFinish;

    const oldWrite = rs.write;
    ...
    return new Promise(
        (resolve, reject) => {
            ...
            // Hack a write onto this socket.
            rs.write = onData;

            rs.once('finish', onFinish = () => {
                _debug('FIFO Got Finish');
        ...
        rs.write = oldWrite;
        rs.removeListener('finish', onFinish);
...
function _runExtraction(pdfStream,
    ...
    return Promise.using(
        ...
        pipeRs => Promise.join(
            _promiseRead(pipeRs),
            execPdfToText(pdfStream, pdf2TxtArgs, pipeRs),
            x => x
        )
    );
}
...
function extractTextFromPdfStream(pdfStream, options = {}) {
    ...
    return Promise.using(
        outputPath,
        (pipePath) => _runExtraction(pdfStream, { pipePath, outputStream }, [...pdf2TxtArgs, '-'])
    );
}
